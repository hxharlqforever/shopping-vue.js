			...
			<div class="content page-order-checkout checkout">
				<div class="js-checkout-address-box">
					<div class="gray-box clear">
						...
						<div class="js-checkout-address" >
							<div class="address-common-table js-multiple-address-panel">
								<ul class="address-item-list clear js-address-item-list">
									<li class="js-choose-address" :class="{'selected-address-item':receiveIndex==index}" @click="choose(index)" v-for='item,index in receiveInfo'>
									...
									<li class="add-address-item js-add-address" @click='addressshow'>
										<p>使用新地址</p>
									</li>
								</ul>
							</div>
						...
						<div class="invoice-detail"> 发票抬头： 
							<div class="radio-box"> 
								<label> 
									...
									<span class="blue-radio" @click='incoiceshow(true)' :class="{'blue-radio-on':incoice.personal}"><a></a></span>  个人
								</label> 
								<label> 
									<input type="radio" class="hide"> 
									<span class="blue-radio" @click='incoiceshow(false)' :class="{'blue-radio-on':!incoice.personal}"><a></a></span>  单位
								</label> 
							</div> 
					...
					<div class="box-inner"> 
						<div class="last-payment clear"> 
							...
							<span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout" @click='submitOrderHandle'> 
								<a>提交订单</a>							
							</span> <span class="prices fn-right">应付金额： <em>¥ {{checkedprice+freight}}.00</em></span> 
						</div> 
					</div>
				</div>	
			</div>
			<addresspop v-if='showpop' @closeone='close'></addresspop>
		</div>
</template>
<script>
import addresspop from '../components/address-pop'
	...
	methods:{
		...
		submitOrderHandle(){
        if(!this.incoice.personal&&!this.incoice.name) return
